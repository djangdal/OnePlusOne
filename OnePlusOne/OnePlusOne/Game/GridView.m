...
- (GridCellView *)gridCellViewForTouchPoint:(CGPoint)touchPoint{
    ...
    return [[self.grid objectAtIndex:cellX] objectAtIndex:cellY];
}

- (NSArray *)neighboursForCell:(GridCellView *)gridCellView{
    NSMutableArray *neighbours = [NSMutableArray new];
    if (gridCellView.x > 0) {
        [neighbours addObject:[[self.grid objectAtIndex:gridCellView.x-1] objectAtIndex:gridCellView.y]];
    }
    if (gridCellView.x < self.gridSize-1) {
        [neighbours addObject:[[self.grid objectAtIndex:gridCellView.x+1] objectAtIndex:gridCellView.y]];
    }
    if (gridCellView.y > 0) {
        [neighbours addObject:[[self.grid objectAtIndex:gridCellView.x] objectAtIndex:gridCellView.y-1]];
    }
