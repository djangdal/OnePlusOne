...
+ (NSArray *)allMissions {
    static NSMutableArray *allMissions;
    if (allMissions) {
        return allMissions;
    }
    allMissions = [NSMutableArray new];
    
    [allMissions addObject:[[Mission alloc] initWithDescription:@"Get 12 points in one game" completionBlock:^void(GameState *gameState, Mission *mission){
        mission.missionState = gameState.totalScore >= 12 ? MissionStateCompleted : MissionStateOngoing;
    }]];
    
    [allMissions addObject:[[Mission alloc] initWithDescription:@"Get more then 40 points in one game" completionBlock:^void(GameState *gameState, Mission *mission){
        mission.missionState = gameState.totalScore > 40 ? MissionStateCompleted : MissionStateOngoing;
    }]];
    
    [allMissions addObject:[[Mission alloc] initWithDescription:@"Place seven 1 tiles before 9 points" completionBlock:^void(GameState *gameState, Mission *mission){
        static int count = 0;
    ...
    [allMissions addObject:[[Mission alloc] initWithDescription:@"Place and merge three 2 tiles" completionBlock:^void(GameState *gameState, Mission *mission){
        static int count = 0;
        if (mission.resetMission) {
            count = 0;
            mission.resetMission = NO;
    ...
    [allMissions addObject:[[Mission alloc] initWithDescription:@"Always have atleast 3 tiles free, and get 30 points" completionBlock:^void(GameState *gameState, Mission *mission){
        __block int count = 0;
        if (mission.missionState != MissionStateFailed) {
    ...
    [allMissions addObject:[[Mission alloc] initWithDescription:@"Get atleast 50 points in one game" completionBlock:^void(GameState *gameState, Mission *mission){
